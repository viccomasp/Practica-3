...
# Aplicamos
wss <- sapply(1:10, function(k) { kmeans(datos_escalados, centers = k, nstart = 25)$tot.withinss })

# Hacemos k-means para valores de k=3 y k=5.
kmeans_result <- kmeans(datos_escalados, centers = 3, nstart = 25)
kmeans_result2 <- kmeans(datos_escalados, centers = 5, nstart = 25)

datos_one_hot$cluster <- as.factor(kmeans_result$cluster)
...
```{r}
ggplot(datos_one_hot, aes(x = tipo_GET, y = status_200, color = cluster)) +
  geom_point() +
  labs(title = "Clusters K-means")
#Falta informació al campus per poder fer els exercicis 8 i 9.
```
